...
ESP BLE MESH 协议栈建立在低功耗蓝牙技术之上,通过 `Adapt Layer`（适配层） 和 BLE 协议栈进行交互。
ESP BLE MESH 协议栈采用的分层的方式设计的，数据包的处理会经过的层次处理顺序是固定的，也就是数据包的处理过程会形成一个`处理流`。`处理流`描述的是数据包经过了哪些层与层处理的先后顺序。从框架图上可以知道主要有4种类型的处理流。
比如：`Bluetooth Low Energy Core` <--> `Adapt Layer` <--> `Advertising Bearer` <--> `Network Layer` <--> `Lower Transport Layer` <--> `Upper Transport Layer` <--> `Access Layer` <--> `Foundation Model Layer` <-->`Model Layer` <--> `API/Event` <--> `Aplication`。
...
## Other:
task：
应用层和协议栈之间的接口设计思想，应用层调用一个 API 或者底层上报 Event都会发送一个消息给 `btc_task`,`btc_task`在去调用应用层或协议栈之前注册的回调函数，调用回调函数的参数是消息中携带的。`btc_task`是一个任务，用于处理应用层和协议栈的请求(消息)。

`adv_task` 这里需要补充一下proxy相关的文档 ！！！
